...
我要开发的是一款 **Windows-only** 的桌面宠物软件，核心形象是“水豚噜噜”。我已经确定技术路线为 **C# + .NET 8 + WPF + Win32 API**，开发环境采用 **Ubuntu + VS Code Remote**，代码托管在 GitHub，持续集成使用 **GitHub Actions 的 `windows-latest` runner** 编译和发布 Windows `exe`。这一组合的关键原因是：WPF 本身就是 Windows-only UI 框架，适合做桌面窗口；WPF `Window` 支持 `Topmost`、`ShowInTaskbar` 等窗口行为配置；透明异形窗口需要 `AllowsTransparency=true` 且 `WindowStyle=None`；而需要进一步处理置顶、点击穿透、工具窗口样式时，可以通过 WPF/Win32 互操作拿到 `HWND`，再调用 Win32 API 和扩展窗口样式完成

因为我的日常开发环境不是 Windows，所以我不会强求在 Ubuntu 本地完整运行 WPF UI；我会把 **状态机、配置、动画调度、数据库访问** 等逻辑尽量放在跨平台可测的 `Core` 层，在 Ubuntu 上完成编写和单元测试；真正依赖 Windows 桌面行为的部分放在 `App` 和 `Win32` 层，并通过 GitHub Actions 的 Windows runner 负责编译和产出 `exe`。对于在 Linux/macOS 上构建 Windows 目标，.NET 官方明确要求设置 `EnableWindowsTargeting=true`。GitHub 官方则推荐在 Actions 中使用 `actions/setup-dotnet` 来保证 .NET 环境的一致性

我会把项目拆成若干可回滚、可验收、可独立合并的 Phase，每个 Phase 都包含：任务清单、在 VS Code 中调用 Codex 的具体命令、最小可验证输出、CI 验收项、提交和 PR 规范、证据要求。发布上我会采用 **SemVer**，通过 Git tag 触发 Release 构建，使用 `dotnet publish -r win-x64` 发布 **single-file** 自包含 Windows 可执行文件。需要注意的是，GitHub 的 `-latest` runner 标签存在渐进迁移机制，因此我会保留“必要时临时 pin 到具体 Windows 版本标签”的回退方案，但当前主线仍使用 `windows-latest`，符合既定路线

## 基线架构与仓库布局

我会坚持“**核心逻辑跨平台，桌面表现 Windows-only**”的架构原则。WPF 项目使用 `net8.0-windows` 目标框架并设置 `UseWPF=true`；WPF/Win32 互操作通过 `WindowInteropHelper.Handle` 或 `HwndSource` 获得宿主窗口句柄；透明窗口与扩展样式由 Win32 扩展样式和 `SetWindowPos` 控制。WPF 这类桌面项目在 SDK 风格项目中应使用 Windows 特定 TFM，例如 `net8.0-windows`，并启用 `UseWPF`
...
flowchart TD
    ...
    H --> I[下载 exe 到 Windows 机手动验收]
    I --> J[提交静态检查结果 日志 CI 链接 手动反馈]
    J --> K[PR 合并 main]
```

上面的工作流之所以成立，是因为 GitHub-hosted runner 会在 Azure 虚拟机上启动全新环境执行作业，`.github/workflows` 是官方规定的工作流位置，而 `workflow_dispatch`、`push.tags` 等事件可以用于手动和标签触发构建
...
| `tests/LuluPet.Core.Tests` | 逻辑层单元测试 | `dotnet test` 能稳定在 CI 中跑通 |
| `assets/pet` | PNG 序列帧资源 | WPF 内置支持 PNG 解码，适合 MVP 序列帧动画 |
| `assets/icons` | 托盘图标、应用图标 | 托盘依赖 `NotifyIcon` |
| `docs` | 计划、验收记录、运行日志 | 把证据和决策固定在仓库里 |
| `.github/workflows` | CI 与发布工作流 | GitHub Actions 官方规定目录 |
...
WPF 的成像管线原生支持 BMP、JPEG、PNG、TIFF、GIF、ICON 等格式；同时，WPF 透明/异形窗口的实现依赖窗口透明能力，因此我在 MVP 阶段优先使用 **PNG 序列帧**，要求带 **完整 Alpha 通道**，避免一开始就把动画制作复杂度推到 Spine/Live2D。`Image` 常见加载方式可直接使用 `BitmapImage`
...
Codex 官方支持 **CLI、本地 IDE 扩展、Windows 原生 App**。在我的场景里，最顺手的是两种方式：一是在 **VS Code Remote 的 Ubuntu 终端**里使用 `codex` / `codex exec`；二是在 VS Code 中安装 **Codex IDE extension**，通过命令面板或侧边栏把当前文件、当前选区、TODO 等直接交给 Codex。Codex 官方说明 IDE 扩展可在 VS Code 及兼容编辑器中使用，CLI 也可以直接在 IDE 终端里运行；`codex exec` 是专门给脚本化和 CI 风格任务准备的非交互模式
...
GitHub Actions 的 workflow 是 YAML 文件，放在 `.github/workflows` 目录；可以通过 `push` 的 `branches` 或 `tags` 过滤，也可以通过 `workflow_dispatch` 手动执行。Artifact 是工作流运行过程中产生并持久化保存的文件集合，适合存放 `exe`、日志和测试输出

### 在 VS Code 中调用 Codex 的标准方式

#### 终端方式

先安装 Codex CLI。官方提供 Linux/macOS 安装脚本，也支持 npm；Windows 另外提供 PowerShell 安装脚本。安装后运行 `codex` 进入交互模式，或使用 `codex exec` 做脚本化执行
...
Codex IDE extension 提供 VS Code 命令面板命令，官方列出的命令包括：`chatgpt.newChat`、`chatgpt.addFileToThread`、`chatgpt.addToThread`、`chatgpt.implementTodo`、`chatgpt.newCodexPanel`、`chatgpt.openSidebar`
...
我会把下面这个脚本放到 `scripts/codex/run-phase.sh`，这样每个 Phase 都可以统一入口执行。`codex exec` 官方支持 `--cd`、`--json`、`--output-last-message`、`--sandbox workspace-write`、`--profile` 等参数；非交互运行时，官方也明确建议使用 `--sandbox workspace-write`
...
如果执行异常，我会优先跑 `codex doctor --json` 保存诊断结果，再修复环境。`codex doctor` 是官方提供的本地诊断命令
...
以上设置符合 .NET Desktop SDK、Windows 特定 TFM、在 Linux/macOS 构建 Windows 目标时设置 `EnableWindowsTargeting`、以及 GitHub Actions 官方推荐的 `setup-dotnet` 用法
...
WPF 官方说明：透明客户端区域需要 `AllowsTransparency=true`，同时 `WindowStyle` 必须是 `None`；`ShowInTaskbar=false` 可以隐藏任务栏按钮；`Topmost=true` 让窗口位于普通窗口之上
...
- XAML 静态检查：窗口属性包含 `WindowStyle=None`、`AllowsTransparency=True`、`Background=Transparent`、`ShowInTaskbar=False`、`Topmost=True`
- CI 成功运行链接与 `LuluPet.exe` artifact 信息
- 手动验收反馈：下载 exe 后确认是否能看到静态噜噜、透明背景、无标题栏、无任务栏图标
...
WPF `Window.DragMove()` 允许在左键按下时拖动窗口；`Window.Top` 等坐标属性可以保存和恢复窗口位置
...
- 配置模型、读写路径、默认值和容错逻辑的代码检查记录
- `settings.json` 样例内容
- 单元测试通过日志
- 手动验收反馈：下载 exe 后确认拖拽、关闭重启位置恢复
...
Windows Forms `NotifyIcon` 官方文档明确说明它适合把没有持续前台界面的后台进程显示在通知区域，支持 `Icon`、`Text`、`Visible`、上下文菜单和事件处理
...
- `NotifyIcon` 初始化、菜单项、关闭拦截和退出释放逻辑的代码检查记录
- CI 成功运行链接与 artifact 信息
- 手动验收反馈：下载 exe 后确认托盘显示、隐藏/恢复、退出进程
...
WPF 支持 `BitmapImage` 和 PNG 解码；动画时序可以使用 `DispatcherTimer`，它与 WPF 的 `Dispatcher` 队列集成，适合 UI 线程定时更新
...
- 动画资源目录结构文本清单
- 帧循环、动作切换、边界条件单元测试日志
- App 层接入 `DispatcherTimer` 或等价调度的代码检查记录
- 手动验收反馈：下载 exe 后确认 Idle/Walk/Sleep 可播放且 UI 不冻结
...
- `docs/acceptance/P5/static-check.md`

**验收证据**

- 状态机转移规则、随机种子/时间源设计的代码检查记录
- 状态切换日志
- 单元测试输出
- 手动验收反馈：下载 exe 后观察 5 分钟，记录是否出现 Idle/Walk/Sleep 切换
...
- `docs/acceptance/P6/static-check.md`

**验收证据**

- 气泡组件、文案加载、异常容错和自动消失逻辑的代码检查记录
- 运行日志
- 手动验收反馈：下载 exe 后确认点击反馈、气泡位置、文案异常时不崩溃
...
WPF/Win32 互操作官方支持通过 `HwndSource` 或 `WindowInteropHelper.Handle` 操作底层窗口；扩展窗口样式文档定义了 `WS_EX_LAYERED`、`WS_EX_TRANSPARENT`、`WS_EX_TOOLWINDOW`；`GetWindowLongPtr`/`SetWindowLongPtr` 用于查询和修改窗口属性；`SetWindowPos` 用于位置、尺寸和 Z 序控制；Windows 官方也定义了 `Run`/`RunOnce` 注册表键用于登录时启动程序
...
- `docs/acceptance/P7/static-check.md`

**验收证据**

- Win32 P/Invoke 签名、扩展样式位操作、边界约束、开机启动写入路径的代码检查记录
- 配置文件与日志中点击穿透、开机启动状态的记录
- 手动验收反馈：下载 exe 后确认点击穿透、边界限制、开机启动开关
...
SQLite 官方文档说明它是嵌入式、无服务器、零配置数据库，直接读写磁盘文件；`CREATE TABLE` 和 WAL 模式均有正式文档支持，适合本地桌面程序的小型持久化需求
...
- `docs/acceptance/P8/static-check.md`
...
- 初始化、读写、重启恢复相关测试日志
- 程序日志中的数据库路径和写入摘要
- 手动验收反馈：下载 exe 后确认首次建库、重启读取、互动日志写入
...
- `settings.json`
- `docs/acceptance/P9/static-check.md`

**验收证据**

- 设置项绑定、实时生效、写回配置、资源缺失回退逻辑的代码检查记录
- 修改前后的 `settings.json` 文本摘要
- 缺失资源容错日志
- 手动验收反馈：下载 exe 后确认设置修改生效并可持久化
...
我会把验收拆成三层，并明确优先级：**静态验收为主，自动化构建/测试为硬门槛，Windows 手动验收为辅**。Codex 在 Ubuntu/Remote 环境中主要负责代码、配置、测试和 CI 产物检查；真正的桌面交互效果由 GitHub Actions 产出 exe 后，我下载到 Windows 本地运行并反馈。

| 层级 | 运行位置 | 关注点 | 必须通过 |
|---|---|---|---|
| 静态验收 | Ubuntu / VS Code / PR diff | 项目结构、XAML 属性、csproj 配置、Win32 P/Invoke 签名、配置读写路径、资源目录、日志与错误处理 | 是 |
| 自动化构建与测试 | Ubuntu / GitHub Actions Windows | `restore`、`build`、`test`、`publish`、artifact 校验是否稳定 | 是 |
| 手动桌面验收 | Windows 真机 | 透明窗口、置顶、拖拽、托盘、点击穿透、动画观感等只能真实运行确认的行为 | 阶段完成前需要反馈 |

`dotnet test` 会构建解决方案并执行测试，测试全通过时返回 0，失败时返回非 0；这非常适合用作 CI gate
...
GitHub 官方推荐在 Actions 中使用 `setup-dotnet`；`upload-artifact` 用于上传构建产物；workflow 触发可以基于 `push` 的分支和标签过滤，也可以使用 `workflow_dispatch` 手动触发
...
这份配置分别使用了 GitHub 官方的 workflow 语法、`setup-dotnet`、artifact 上传机制，以及 .NET 官方的 `dotnet publish` single-file/self-contained 方式。`windows-latest` 是标准 GitHub-hosted Windows runner 标签，但官方也提示 `-latest` 会渐进迁移到更新镜像，所以如果某次 runner 迁移影响稳定性，我会把发布工作流短期 pin 到具体 Windows 标签，等兼容验证完再切回
...
| 证据归档 | 将静态检查记录、日志、Hash、artifact 链接、手动反馈放回仓库或 PR 描述 |
...
这份配置对应 .NET Desktop SDK 的 WPF 项目要求：Windows 特定 TFM、`UseWPF=true`、可选 `WinExe` 输出，以及在非 Windows 系统构建 Windows 目标时需要的 `EnableWindowsTargeting=true`
...
这段配置遵循 WPF 官方窗口透明与桌面行为规则：透明客户端区域要求 `AllowsTransparency=true` 且 `WindowStyle=None`，`ShowInTaskbar=false` 可隐藏任务栏按钮，`Topmost=true` 将窗口放到 topmost 组，`DragMove()` 则是官方提供的无标题栏拖动入口
...
如果我在 WPF UI 层使用 `DispatcherTimer` 来驱动 `Tick()`，它会与 WPF Dispatcher 队列集成，适合做轻量 UI 动画调度；图片加载则可用 `BitmapImage`
...
这里分别使用了 `WindowInteropHelper.Handle` 获取 `HWND`，`GetWindowLongPtr`/`SetWindowLongPtr` 读取与写入扩展窗口样式，以及 `SetWindowPos` 修正 Z 序和 topmost 状态。官方文档也明确说明了 `WS_EX_LAYERED`、`WS_EX_TRANSPARENT`、`WS_EX_TOOLWINDOW` 的语义
...
如果我启用开机启动，我会把可执行命令写入 Windows `Run` 注册表键；这属于当前用户登录时自动启动的标准入口
...
SQLite 官方文档支持 `CREATE TABLE IF NOT EXISTS`，WAL 模式则通过 `PRAGMA journal_mode=WAL` 启用。SQLite 本身是嵌入式、无服务器、零配置数据库，非常适合桌面程序的本地状态持久化

### 每个 Phase 结束时我必须提交的证据

| 类别 | 最低要求 |
|---|---|
| 静态检查 | 至少 1 份 `docs/acceptance/PX/static-check.md`，记录关键文件、关键属性、配置路径和风险点 |
| 日志 | 至少 1 份 `docs/acceptance/PX/*.log` 或 `*.jsonl` |
| CI | 至少 1 个成功 workflow run 的 artifact 说明 |
| 产物 | 如果该 Phase 影响可执行文件，必须提供 exe artifact 名称 |
| 代码 | 提交哈希、分支名、PR 链接 |
| 测试 | 本地验证命令和结果摘要 |
| 手动反馈 | 如果该 Phase 影响桌面表现，记录我下载 exe 后的 Windows 本地运行反馈 |

### 下一步 Codex 执行指令模板

下面这个模板是我后续每个 Phase 都会直接复用的提示词格式。`codex exec` 官方适合“脚本化 / CI 风格 / 无人工交互”的执行，因此我会用它来驱动标准 Phase 任务
...
- 测试：通过 / 失败
- 静态验收：通过 / 失败
- 手动验收：待下载 exe 验收 / 通过 / 失败

【产物清单】
- exe / 数据库 / 日志 / 配置文件 / 静态检查记录

【证据路径】
- docs/acceptance/P{X}/...
- GitHub Actions artifact: ...
- 手动验收反馈：待填写 / docs/acceptance/P{X}/manual-feedback.md
