...
LuluPet 是一个 Windows 桌面宠物应用。它使用 WPF 做透明置顶窗口，通过 PNG 序列帧播放宠物动作，并用托盘菜单管理显示、隐藏、设置、教程、点击穿透和开机启动。除桌宠交互外，当前版本还提供剪切板历史、文件中转站和专注提醒面板。
...
- 快捷键：`I` 强制 Idle，`W` 强制 Walk，`S` 强制 Sleep。
- 主窗口激活时按 `F`：在噜噜右侧打开提醒面板，再按 `F` 可关闭。
- 主窗口激活时按 `C`：打开剪切板历史面板。
- 主窗口激活时按 `D`：打开文件中转站面板。

点击穿透开启后，`F` / `C` / `D` 面板快捷键不会生效；关闭点击穿透后恢复。
...
番茄钟开启后会在“专注 -> 休息 -> 下一轮专注”之间循环。番茄钟结束、休息结束、喝水提醒、起立休息提醒触发时，噜噜会显示对应气泡，并走到当前所在屏幕的最近边缘绕屏幕一周。绕屏过程中气泡会保持显示；点击或拖动噜噜会停止本次绕屏并恢复 idle。
...
按 `C` 打开的剪切板面板提供最近 100 条文本历史：

- 复制文本后会自动记录到历史中。
- 点击某条历史会立即把该文本写回系统剪切板。
- 重复文本会移动到最上方，不会重复占用列表。
- 历史会保存到本地，重启后恢复最近 100 条。

剪切板历史文件位置：

```text
%LOCALAPPDATA%\LuluPet\data\clipboard-history.json
```

## 文件中转站

按 `D` 打开的文件中转站用于临时暂存文件：

- 拖入文件：复制到中转站文件夹，不移动原文件。
- 在 D 面板中按 `Ctrl+V`：如果剪切板是文件，会复制文件到中转站；如果剪切板是未保存的图片，会保存为 PNG。
- 双击文件：用系统默认程序打开。
- 右键文件：复制该文件，可到资源管理器或其他位置粘贴。
- 同名文件会自动追加序号。
- 中转站文件夹可在设置面板里自定义。
...
```text
%LOCALAPPDATA%\LuluPet\data\Transit
```
...
`设置` 和 `教程` 都会以噜噜右侧的气泡面板显示。设置项会立即生效并持久化到 `settings.json`：
...
- `interaction.walkMode`：散步模式开关；关闭时噜噜保持在当前位置，开启后才会随机 walk
...
- `clipboard-history.json`：最近 100 条剪切板文本历史
- `Transit/`：文件中转站默认文件夹
...
```text
src/
  LuluPet.App/      WPF 桌面应用、主窗口、气泡面板、托盘、交互逻辑
  LuluPet.Core/     动画播放器、状态机、配置、剪切板、文件中转、对白、SQLite 持久化
...
- Release asset：`LuluPet-win-x64-v版本号.zip`，例如 `LuluPet-win-x64-v1.2.0.zip`
